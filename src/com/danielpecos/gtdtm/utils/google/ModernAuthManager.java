...
public class ModernAuthManager implements AuthManager {
	...
	public void authResult(int resultCode, Intent results) {
		boolean retry = false;
		if (results == null) {
			Log.e(TaskManager.TAG, "No auth token!!");
		} else {
		...
		if (authToken == null && retry) {
			Log.i(TaskManager.TAG, "Retrying to get auth result");
			doLogin(lastAccount);
			return;
		}
	...
	public void invalidateAndRefresh(final AuthCallback callback) {
		...
		activity.runOnUiThread(new Runnable() {
			public void run() {
				...
				accountChooser.chooseAccount(activity, new AccountChooser.AccountHandler() {
					@Override
